 ...
 * accepted ownership of the obligation.
 */
const RUNTIME_REQUEST_STATUSES = new Set([
  "received",
  "accepted",
  "queued",
  "processing",
 ...
 * OWNERSHIP MODEL.
 *
 * The business-event outbox answers exactly ONE question: did this business
 * event successfully hand off to Omni-Comms? Once a durable Omni-Comms
 * request exists, the handoff is complete and the outbox row is `processed`
 * — permanently. Everything after that (configuration blockers, rendering,
 * dispatch, provider delivery) is owned by the request / message / job
 * lifecycle and is surfaced in Activity from those records.
 *
...
export function classifyRuntimeOutcome(outcome: RuntimeOutcome): IngestClassification {
  ...
  // 1. Durable request exists → handoff complete, whatever the request says.
