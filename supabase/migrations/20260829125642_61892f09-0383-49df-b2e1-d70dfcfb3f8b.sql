...
CREATE OR REPLACE FUNCTION public.ce_violation_return_to_queue_v1(
  p_violation_id uuid, p_reason text, p_notes text
...
BEGIN
  ...
  SELECT v.assigned_queue_id, v.assigned_to_name INTO v_queue_id, v_prev_name
  ...
  SET assigned_to_user_id = NULL, assigned_to_name = NULL,
...
REVOKE ALL ON FUNCTION public.ce_violation_return_to_queue_v1(uuid,text,text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.ce_violation_return_to_queue_v1(uuid,text,text) TO authenticated, service_role;
