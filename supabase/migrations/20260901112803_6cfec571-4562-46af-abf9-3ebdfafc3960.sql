-- 1. Per-basket notification wording
ALTER TABLE public.bn_workbasket
  ADD COLUMN IF NOT EXISTS notify_title text,
  ADD COLUMN IF NOT EXISTS notify_body text,
  ADD COLUMN IF NOT EXISTS notify_action_label text;

COMMENT ON COLUMN public.bn_workbasket.notify_title IS 'In-app arrival notification title template. Tokens: {claim_number} {benefit} {status} {step} {basket_name} {due_date} {priority}';
COMMENT ON COLUMN public.bn_workbasket.notify_body IS 'In-app arrival notification body template. Same tokens as notify_title.';
COMMENT ON COLUMN public.bn_workbasket.notify_action_label IS 'Call-to-action label shown on the notification, e.g. Review eligibility.';

-- 2. Idempotency: one arrival notification per user per assignment
CREATE UNIQUE INDEX IF NOT EXISTS ux_in_app_notifications_bn_wb_arrival
  ON public.in_app_notifications (user_id, ((metadata ->> 'assignment_id')))
  WHERE notification_type = 'BN_WORKBASKET_ARRIVAL';

-- 3. Token renderer
CREATE OR REPLACE FUNCTION public.bn_render_workbasket_notification(
  p_template text,
  p_tokens jsonb
) RETURNS text
LANGUAGE plpgsql
...
BEGIN
  ...
  FOR v_key IN SELECT jsonb_object_keys(coalesce(p_tokens, '{}'::jsonb)) LOOP
    v_out := replace(v_out, '{' || v_key || '}', coalesce(p_tokens ->> v_key, ''));
  END LOOP;
  RETURN v_out;
END;
$$;

-- 4. Fan-out on basket arrival
...
BEGIN
  ...
  BEGIN
    ...
    v_title := public.bn_render_workbasket_notification(
      nullif(v_basket.notify_title, ''),
      v_tokens
    );
    ...
    v_body := public.bn_render_workbasket_notification(
      nullif(v_basket.notify_body, ''),
      v_tokens
    );
...
DROP TRIGGER IF EXISTS zz_bn_claim_queue_assignment_notify ON public.bn_claim_queue_assignment;
CREATE TRIGGER zz_bn_claim_queue_assignment_notify
AFTER INSERT ON public.bn_claim_queue_assignment
FOR EACH ROW EXECUTE FUNCTION public.bn_notify_workbasket_arrival();

GRANT EXECUTE ON FUNCTION public.bn_render_workbasket_notification(text, jsonb) TO authenticated, service_role;