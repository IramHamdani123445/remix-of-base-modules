...
BEGIN
  ...
  FOREACH v_item IN ARRAY coalesce(p_recipients, '{}'::text[]) LOOP
    ...
    v_item := CASE WHEN v_ch = 'email' THEN lower(btrim(v_item)) ELSE regexp_replace(btrim(v_item), '[\s()\-\.]', '', 'g') END;
  ...
  UPDATE public.omni_comms_channel_setting
     SET controlled_test_delivery_enabled = coalesce(p_enabled,false),
         ...
         controlled_test_approval_expires_at = CASE WHEN coalesce(p_enabled,false) THEN now() + make_interval(hours => v_hours) ELSE NULL END,
...
ALTER FUNCTION public.omni_comms_channel_test_delivery_set_approval(uuid,uuid,text,boolean,text[],integer,integer,integer) OWNER TO postgres;