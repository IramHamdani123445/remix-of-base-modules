-- DEF-8: internal-delivery channels can never satisfy provider credential
-- verification or provider-confirmed binding verification, because neither
-- capability exists for them. Mark them not_applicable rather than failed.
CREATE OR REPLACE FUNCTION public.omni_comms_priv_binding_verification_supported(p_channel text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
SET search_path TO 'pg_catalog', 'public'
AS $fn$
  -- Mirrors omni_comms_priv_verify_binding_configuration, which raises
  -- channel_verification_not_supported for every channel except email.
  SELECT btrim(coalesce(p_channel, '')) = 'email';
$fn$;

REVOKE ALL ON FUNCTION public.omni_comms_priv_binding_verification_supported(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.omni_comms_priv_binding_verification_supported(text) TO service_role;
...
DECLARE
  ...
  v_cred text;
  v_cred_code text;
  v_cred_required boolean;
  v_verify_supported boolean;
BEGIN
  ...
  v_cred_required := (a <> 'null'::jsonb)
                     AND coalesce((a->>'required_credential_count')::int, 0) > 0;
  v_ok := (a <> 'null'::jsonb)
          AND (v_cred = 'verified' OR v_cred_code = 'restricted_api_key');
  v_checks := v_checks || jsonb_build_array(jsonb_build_object(
    'code','provider_credentials_verified','label','Credentials verified',
    'state', CASE
      WHEN v_ok THEN 'passed'
      WHEN NOT v_cred_required THEN 'not_applicable'
      ELSE 'failed' END,
    'detail', CASE
      ...
      WHEN NOT v_cred_required
        THEN 'This provider account requires no external credential, so provider credential verification does not apply.'
  ...
  v_verify_supported := public.omni_comms_priv_binding_verification_supported(p_channel);
  v_ok := coalesce(b->>'verification_status','') = 'verified';
  v_checks := v_checks || jsonb_build_array(jsonb_build_object(
    'code','binding_verification','label','Binding verification',
    'state', CASE
      WHEN v_ok THEN 'passed'
      WHEN NOT v_verify_supported THEN 'not_applicable'
      ELSE 'failed' END,
    'detail', CASE
      WHEN v_ok THEN 'Binding carries a provider-confirmed verified state.'
      WHEN NOT v_verify_supported
        THEN 'Provider-confirmed binding verification is not offered for this channel, so it does not apply.'
      ELSE 'Binding must carry a provider-confirmed verified state.' END));
