...
DECLARE
  ...
  v_lines jsonb; v_account uuid; v_letter text; v_stationery jsonb;
BEGIN
  ...
  FOR v_job IN
    SELECT j.*, m.rendered_subject, m.rendered_text, m.rendered_html,
           ...
           m.template_family_id, m.template_version_id,
     ...
     WHERE j.channel = 'print'
       AND j.mode = 'queued'
       AND (j.attempt_count < 3 OR
            (j.hold_reason = 'automatic_print_lease_recovery' AND j.attempt_count < 6))
  ...
  LOOP
    ...
    v_lines := CASE
      WHEN jsonb_typeof(v_job.destination_snapshot->'postal_address_lines') = 'array'
        THEN v_job.destination_snapshot->'postal_address_lines'
      WHEN jsonb_typeof(v_job.destination_snapshot->'address_lines') = 'array'
        THEN v_job.destination_snapshot->'address_lines'
      WHEN jsonb_typeof(v_job.destination_snapshot->'print') = 'string'
        THEN to_jsonb(regexp_split_to_array(v_job.destination_snapshot->>'print', E'\\r?\\n'))
      ELSE '[]'::jsonb END;
    ...
    v_stationery := public.omni_comms_priv_print_stationery_effective(
                      v_job.organization_id, v_job.msg_department_id);

    v_token := encode(extensions.gen_random_bytes(24),'hex');
    SELECT coalesce(max(a.attempt_number), 0) + 1 INTO v_attempt_no
      FROM public.omni_comms_delivery_attempt a
     WHERE a.dispatch_job_id = v_job.id;
    v_letter := 'LTR-' || upper(substr(replace(v_job.message_id::text,'-',''),1,12));

    -- Governed lifecycle ladder: a HELD print job must pass through
    -- ready -> leased before it may enter processing. Only the recovery
    -- path (retry_wait) is permitted to jump straight to processing.
    IF v_job.status = 'held' THEN
      UPDATE public.omni_comms_dispatch_job
         SET status = 'ready', hold_reason = NULL, is_runnable = true, updated_at = now()
       WHERE id = v_job.id;
      UPDATE public.omni_comms_dispatch_job
    ...
    UPDATE public.omni_comms_dispatch_job
       SET status = 'processing', is_runnable = false,
           hold_reason = CASE WHEN v_job.hold_reason='automatic_print_lease_recovery'
                              AND v_job.status='retry_wait'
                              THEN v_job.hold_reason ELSE NULL END,
           lock_token = v_token, locked_at = now(), locked_by = v_worker,
    ...
    VALUES (
      ...
      jsonb_build_object('channel','print','mode','queued',
                         ...
                         'letterhead_source', v_stationery->>'letterhead_source',
                         'print_footer_source', v_stationery->>'print_footer_source',
                         'correlation_id', nullif(v_corr,'')))
    RETURNING id INTO v_attempt_id;

    v_claimed := v_claimed + 1;
    v_claims := v_claims || jsonb_build_object(
      'attempt_id', v_attempt_id, 'claim_token', v_token,
      'attempt_number', v_attempt_no, 'message_id', v_job.message_id,
      'organization_id', v_job.organization_id, 'letter_reference', v_letter,
      'subject', v_job.rendered_subject, 'text_body', v_job.rendered_text,
      'html_body', v_job.rendered_html, 'recipient_display', v_job.recipient_display,
      'recipient_reference', v_job.recipient_reference, 'postal_address_lines', v_lines,
      'issuing_authority', v_job.channel_setting_snapshot->>'issuing_authority',
      'stationery', v_stationery,
      'production_account_id', v_account, 'event_code', v_job.event_code);
  END LOOP;
