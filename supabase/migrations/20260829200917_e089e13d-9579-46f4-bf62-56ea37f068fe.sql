-- 1. Canonical Internal Audit communication owner department resolver.
CREATE OR REPLACE FUNCTION public.ia_comms_owner_department_id()
RETURNS uuid
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  IF v_id IS NULL THEN
    RAISE EXCEPTION USING
      ERRCODE = 'P0001',
      MESSAGE = 'IA_COMMS_OWNER_DEPARTMENT_UNRESOLVED',
      HINT    = 'core_department with code INTERNAL_AUDIT is required for Internal Audit communication routing';
  END IF;
...
GRANT EXECUTE ON FUNCTION public.ia_comms_owner_department_id() TO authenticated, service_role;

-- 2. Domain classifier: is a uuid a communication-owner department, an audited
--    department, or unknown? Used by the emitter guard and by regression tests.
CREATE OR REPLACE FUNCTION public.ia_comms_department_domain(p_department_id uuid)
RETURNS text
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $function$
  SELECT CASE
    WHEN p_department_id IS NULL THEN 'none'
    WHEN EXISTS (SELECT 1 FROM public.core_department d WHERE d.id = p_department_id) THEN 'core_department'
    WHEN EXISTS (SELECT 1 FROM public.ia_departments d WHERE d.id = p_department_id) THEN 'ia_department'
    ELSE 'unknown'
  END;
$function$;

GRANT EXECUTE ON FUNCTION public.ia_comms_department_domain(uuid) TO authenticated, service_role;

-- 3. Emitter: routing department is ALWAYS the Internal Audit owner
--    (core_department). Any department passed by a business command is treated
--    as the AUDITED department business context and preserved in the payload.
...
BEGIN
  ...
  v_payload := coalesce(v_proj->'payload', '{}'::jsonb);
