...
DECLARE
  ...
  v_adapter text;
  v_hash_err text;
  v_rel_snapshot public.omni_comms_channel_release_control%ROWTYPE;
  v_req           record;
  v_existing      int;
  v_msg           jsonb;
  v_job           jsonb;
...
BEGIN
  ...
  FOR v_job IN SELECT * FROM jsonb_array_elements(p_jobs) LOOP
    ...
    v_rel_snapshot := NULL;
    IF v_proposed AND v_authz IS NULL THEN
      v_rel_snapshot := public.omni_comms_priv_channel_release_effective(
        p_organization_id, v_msg_dept, v_job->>'channel');
      IF v_rel_snapshot.id IS NULL OR v_rel_snapshot.release_fingerprint IS NULL THEN
        v_authz := 'release_snapshot_unavailable';
      END IF;
    END IF;
    ...
    INSERT INTO public.omni_comms_dispatch_job (
      ...
      v_reason,
      CASE WHEN v_effective THEN v_rel_snapshot.id END,
      CASE WHEN v_effective THEN now() END,
      CASE WHEN v_effective THEN v_rel_snapshot.release_version END,
      CASE WHEN v_effective THEN v_rel_snapshot.release_state END,
      CASE WHEN v_effective THEN v_rel_snapshot.release_fingerprint END,
      CASE WHEN v_effective THEN v_rel_snapshot.release_expires_at END
    );
    ...
    INSERT INTO public.omni_comms_message_event (
      ...
      jsonb_build_object(
        ...
        'recipient_hash_head', left(coalesce(v_recip_hash,''), 12),
        'adapter_used',       v_adapter,
        'hash_error',         v_hash_err
      ),
      v_req.correlation_id, 'system', p_actor_id::text
    );
  END LOOP;
