-- ===========================================================================
-- Omni-Comms Reference Seed Pack — catalogue v2
-- Lifecycle ordering, structural conflict detection, reconciliation,
-- deterministic pinned layouts, extended status.
-- ===========================================================================

-- ---------------------------------------------------------------------------
-- 1. Deterministic reference layouts (seed-owned, published immutable version)
-- ---------------------------------------------------------------------------
DO $$
DECLARE
  r record;
...
BEGIN
  FOR r IN
    SELECT * FROM (VALUES
      ('OMNI_REF_EMAIL',  'Omni-Comms Reference Email Layout',  'EMAIL'),
      ('OMNI_REF_SMS',    'Omni-Comms Reference SMS Layout',    'SMS'),
      ('OMNI_REF_IN_APP', 'Omni-Comms Reference In-App Layout', 'IN_APP')
    ) AS t(code, name, kind)
  LOOP
    SELECT id INTO v_layout_id FROM public.core_template_layout WHERE code = r.code;
    IF v_layout_id IS NULL THEN
      INSERT INTO public.core_template_layout
        (code, name, description, layout_kind, has_letterhead, is_active,
         show_page_numbers, show_generated_date, show_doc_reference,
         created_by, updated_by)
      VALUES
        (r.code, r.name,
         'reference_seed: non-production Omni-Comms reference layout (catalogue v2).',
         r.kind, false, true, false, false, false,
         'omni_comms_reference_seed', 'omni_comms_reference_seed')
      RETURNING id INTO v_layout_id;
    END IF;

    IF NOT EXISTS (
      SELECT 1 FROM public.core_template_layout_version
       WHERE layout_id = v_layout_id AND status = 'published'
    ) THEN
      INSERT INTO public.core_template_layout_version
        (layout_id, version_number, slots, wrapper_html, checksum, status, published_at)
      VALUES
        (v_layout_id, 1,
         '[{"code":"content_body","order":1,"required":true}]'::jsonb,
         '<div data-omni-ref-layout="1">{{content_body}}</div>',
         encode(pg_catalog.sha256(convert_to(r.code || ':v1', 'UTF8')), 'hex'),
         'published', now());
    END IF;
  END LOOP;
END $$;
...
-- ---------------------------------------------------------------------------
-- 2. Catalogue v2 overlay (deterministic reference layouts)
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.omni_comms_priv_reference_seed_catalogue_v2()
RETURNS jsonb
LANGUAGE sql
STABLE
...
AS $fn$
  SELECT jsonb_set(
           jsonb_set(
             public.omni_comms_priv_reference_seed_catalogue(),
             '{catalogue_version}', '2'::jsonb, true),
           '{layouts}',
           jsonb_build_object(
             'email',  'OMNI_REF_EMAIL',
             'sms',    'OMNI_REF_SMS',
             'in_app', 'OMNI_REF_IN_APP'),
           true)
$fn$;

REVOKE ALL ON FUNCTION public.omni_comms_priv_reference_seed_catalogue_v2() FROM PUBLIC;

-- ---------------------------------------------------------------------------
-- 3. Runner v2 — lifecycle ordering + conflict detection + reconciliation
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  FOR it IN SELECT * FROM jsonb_array_elements(c -> 'accounts') LOOP
    ...
    IF FOUND THEN
      v_note_reason := NULL;
      IF v_provider_id IS NULL OR rec.provider_id IS DISTINCT FROM v_provider_id THEN
  ...
  FOR it IN SELECT * FROM jsonb_array_elements(c -> 'events') LOOP
    ...
    -- ---- 1. event definition (draft first; activated only after contract) --
    v_event_id := NULL;
    ...
    -- ---- 5. activate event definition ONLY after a published contract -----
    IF v_write THEN
    ...
    IF FOUND THEN
      ...
      ELSIF rec.event_definition_id IS DISTINCT FROM v_event_id THEN
    ...
    FOR ch IN SELECT * FROM jsonb_array_elements(it -> 'channels') LOOP
      ...
      IF v_family_id IS NOT NULL AND rec.id IS NOT NULL THEN
      ...
      ELSIF NOT v_write OR v_family_id IS NULL THEN
        v_planned := v_planned + 1;
      ...
      ELSE
        ...
        -- System-seed authorship (created_by NULL = omni_comms_reference_seed
        -- system principal); the operator is the independent approver.
      ...
      IF v_write AND (v_family_id IS NULL OR v_sender_id IS NULL OR v_version_id IS NULL) THEN
        v_blocked := v_blocked + 1;
...
-- ---------------------------------------------------------------------------
-- 4. Public RPCs (preview / apply / reconcile / status)
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  RETURN public.omni_comms_priv_reference_seed_run_v2(v_uid, p_organization_id, 'preview', NULL);
END;
$fn$;
...
BEGIN
  v_uid := public.omni_comms_priv_require_capability('configure');
  IF p_confirm_non_production IS DISTINCT FROM true THEN
    RAISE EXCEPTION 'OC400 confirmation_required'
      USING ERRCODE = 'P0001', DETAIL = 'non_production_confirmation_required';
  END IF;
  PERFORM public.omni_comms_priv_reference_seed_assert_safe(p_organization_id);
  RETURN public.omni_comms_priv_reference_seed_run_v2(v_uid, p_organization_id, 'apply', p_correlation_id);
END;
$fn$;

CREATE OR REPLACE FUNCTION public.omni_comms_reference_seed_reconcile(
  p_organization_id uuid,
  p_confirm_non_production boolean,
  p_correlation_id text
)
...
BEGIN
  ...
  END IF;
  PERFORM public.omni_comms_priv_reference_seed_assert_safe(p_organization_id);
  RETURN public.omni_comms_priv_reference_seed_run_v2(v_uid, p_organization_id, 'reconcile', p_correlation_id);
END;
$fn$;
...
BEGIN
  ...
  -- required layout slots that are not reserved content slots and have no asset
  ...
  -- structural conflicts, computed without mutating anything
...
REVOKE ALL ON FUNCTION public.omni_comms_reference_seed_reconcile(uuid, boolean, text) FROM PUBLIC;
...
GRANT EXECUTE ON FUNCTION public.omni_comms_reference_seed_reconcile(uuid, boolean, text) TO authenticated;
