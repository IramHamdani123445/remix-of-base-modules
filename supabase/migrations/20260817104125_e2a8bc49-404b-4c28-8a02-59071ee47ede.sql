...
BEGIN
  ...
  IF v_msg.organization_id IS DISTINCT FROM NEW.organization_id THEN RAISE EXCEPTION 'OC422 dispatch_message_org_mismatch' USING ERRCODE='P0001'; END IF;
  IF v_msg.request_id IS DISTINCT FROM NEW.request_id THEN RAISE EXCEPTION 'OC422 dispatch_message_request_mismatch' USING ERRCODE='P0001'; END IF;
  IF v_msg.channel <> NEW.channel THEN RAISE EXCEPTION 'OC422 dispatch_message_channel_mismatch' USING ERRCODE='P0001'; END IF;
  IF NEW.mode='dry_run' AND NEW.is_runnable=true THEN RAISE EXCEPTION 'OC422 dry_run_not_runnable' USING ERRCODE='P0001'; END IF;
  ...
  IF TG_OP='UPDATE' AND NEW.status<>OLD.status THEN
    ...
    ELSIF OLD.status='retry_wait' AND NEW.status='processing' THEN
      IF OLD.channel<>'print'
         ...
         OR NEW.lock_token IS NULL OR NEW.locked_by IS NULL OR NEW.lease_expires_at IS NULL THEN
    ...
    ELSIF OLD.status='processing' AND NEW.status='held' THEN
      IF NOT v_worker THEN RAISE EXCEPTION 'OC403 dispatch_worker_context_required' USING ERRCODE='P0001'; END IF;
      ...
      END IF;
    ELSIF OLD.status='held' AND OLD.hold_reason='reconciliation_required' AND NEW.status IN ('completed','failed') THEN
      IF NOT v_verified THEN RAISE EXCEPTION 'OC403 verified_callback_context_required' USING ERRCODE='P0001'; END IF;
      IF NEW.is_runnable IS NOT FALSE THEN RAISE EXCEPTION 'OC422 invalid_reconciliation_resolution' USING ERRCODE='P0001'; END IF;
    ELSE RAISE EXCEPTION 'OC422 invalid_dispatch_transition' USING ERRCODE='P0001';
    END IF;
  END IF;
  RETURN NEW;
END;
