-- BN Uprating Epic 4 — backend-owned post-execution operational queue (read only)
...
BEGIN
  ...
  CREATE TEMP TABLE IF NOT EXISTS _bnupr_queue_tmp ON COMMIT DROP AS SELECT 1 WHERE false;
  ...
  SELECT
    ...
    COALESCE((SELECT jsonb_agg(x) FROM (
      SELECT jsonb_build_object(
        ...
        'last_activity_at', GREATEST(COALESCE(l.rolled_back_at, l.updated_at),
                                     COALESCE(l.reconciled_at, l.updated_at),
                                     COALESCE(l.communications_issued_at, l.updated_at),
                                     COALESCE(l.schedules_rebuilt_at, l.updated_at),
                                     l.updated_at)) AS x
        FROM labelled l
