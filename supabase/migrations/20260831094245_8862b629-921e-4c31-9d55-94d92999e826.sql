CREATE INDEX IF NOT EXISTS idx_ce_follow_up_actions_due_status ON public.ce_follow_up_actions (due_date, status);
CREATE INDEX IF NOT EXISTS idx_ce_notices_due_response ON public.ce_notices (due_response_date);
CREATE INDEX IF NOT EXISTS idx_ce_inspections_scheduled ON public.ce_inspections (scheduled_date);
CREATE INDEX IF NOT EXISTS idx_ce_violations_due_date ON public.ce_violations (due_date);
...
BEGIN
  ...
  v_thresholds := jsonb_build_object(
    ...
    'horizon_from', v_h_from, 'horizon_to', v_h_to,
  ...
  WHERE COALESCE(v.is_deleted,false) = false AND v.due_date::date BETWEEN v_h_from AND v_h_to;
  ...
  WHERE COALESCE(c.is_deleted,false) = false AND c.target_resolution_date::date BETWEEN v_h_from AND v_h_to;
  ...
  WHERE n.due_response_date::date BETWEEN v_h_from AND v_h_to;
  ...
  WHERE i.due_date BETWEEN v_h_from AND v_h_to;
  ...
  WHERE COALESCE(f.is_deleted,false) = false AND f.due_date::date BETWEEN v_h_from AND v_h_to;
  ...
  WHERE s.scheduled_date::date BETWEEN v_h_from AND v_h_to;
  ...
  -- deadline breaches & imminent deadlines (capped per area, most overdue first)
  INSERT INTO _mon_ex
  SELECT CASE WHEN d.due_date < v_today - 7 THEN 'Critical'
         ...
         d.route
  FROM (
    SELECT *, ROW_NUMBER() OVER (PARTITION BY area ORDER BY due_date) rn
    FROM _mon_dl WHERE closed = false AND due_date <= v_today + 1
  ) d
  WHERE d.rn <= 150;
  ...
  SELECT jsonb_build_object(
    ...
    'by_area', COALESCE((
      SELECT jsonb_agg(jsonb_build_object('area', area, 'breached', b, 'due_soon', s) ORDER BY b DESC)
      FROM (SELECT area,
  ...
  BEGIN
    ...
    FROM (
      SELECT jsonb_build_object('d', d.d::date, 'category', COALESCE(t.category,'Other'), 'count', COUNT(v.id)) x
  ...
  RETURN jsonb_build_object(
    ...
    'exceptions_truncated', (SELECT COUNT(*) FROM _mon_ex) > 400,
    'sla_summary', v_sla,
