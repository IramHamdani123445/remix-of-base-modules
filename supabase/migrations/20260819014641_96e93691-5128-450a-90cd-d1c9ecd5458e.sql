-- 1. Governed In-App bulk / mark-all read with Omni-Comms engagement evidence.
CREATE OR REPLACE FUNCTION public.omni_comms_in_app_record_engagement_bulk(
...
BEGIN
  ...
  END IF;
  IF p_notification_ids IS NOT NULL AND array_length(p_notification_ids, 1) > 500 THEN
    RAISE EXCEPTION 'OC422 validation_error'
      USING ERRCODE='P0001', DETAIL='bulk_selection_too_large';
  END IF;
  ...
  LOOP
    ...
    IF n.omni_comms_message_id IS NOT NULL AND n.omni_comms_request_id IS NOT NULL THEN
      SELECT organization_id INTO v_org
        FROM public.omni_comms_message WHERE id = n.omni_comms_message_id;
      ...
      INSERT INTO public.omni_comms_message_event (
        ...
        'callback_opened', v_seq,
        'Recipient opened the in-app notification.',
        'recipient', jsonb_build_object('channel', 'in_app', 'bulk', true));
  ...
  RETURN jsonb_build_object(
    'affected', v_affected,
    'omni_evidence_written', v_evidence,
    'scope', CASE WHEN p_notification_ids IS NULL THEN 'all_unread' ELSE 'selection' END);
END;
$function$;

REVOKE ALL ON FUNCTION public.omni_comms_in_app_record_engagement_bulk(uuid[]) FROM public;
GRANT EXECUTE ON FUNCTION public.omni_comms_in_app_record_engagement_bulk(uuid[]) TO authenticated;

-- 2. In-App dispatch uses the authoritative channel release resolver.
