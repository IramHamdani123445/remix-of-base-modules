-- Claim queued PRINT jobs for artefact production.
...
-- Record the produced artefact. The auto-ensure trigger turns an accepted
-- attempt into a physical print item with the postal address snapshot.
CREATE OR REPLACE FUNCTION public.omni_comms_priv_print_production_complete(
  p_attempt_id uuid, p_claim_token text, p_status text, p_artefact jsonb DEFAULT NULL,
...
BEGIN
  ...
  IF p_status = 'accepted'
     AND coalesce(btrim(v_a->>'artefact_path'),'') = '' THEN
  ...
  ELSE
    UPDATE public.omni_comms_dispatch_job
       SET status = CASE WHEN v_att.attempt_number >= 3 THEN 'failed' ELSE 'retry_wait' END,
           is_runnable = false,
           next_attempt_at = CASE WHEN v_att.attempt_number >= 3 THEN NULL
                                  ELSE now() + (interval '1 minute' * v_att.attempt_number) END,
           completed_at = CASE WHEN v_att.attempt_number >= 3 THEN now() ELSE NULL END,
           lock_token=NULL, locked_at=NULL, locked_by=NULL, lease_expires_at=NULL,
           updated_at=now()
     WHERE id = v_att.dispatch_job_id;
    IF v_att.attempt_number >= 3 THEN
