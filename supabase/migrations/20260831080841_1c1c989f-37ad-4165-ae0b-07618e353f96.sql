...
BEGIN
  ...
  -- ---------------- workflow (canonical compliance escalation stages)
  workflow AS (
  ...
  -- ---------------- pipeline (compliance handoff + legal case stages)
  pipe_raw AS (
  ...
  -- ---------------- priority matters
  priority AS (
    SELECT COALESCE(jsonb_agg(x ORDER BY ord), '[]'::jsonb) AS j FROM (
      SELECT jsonb_build_object(
  ...
  officers AS (
    ...
    FROM (
      SELECT COALESCE(c.assigned_legal_officer_id::text, 'Unassigned') officer,
             count(*) FILTER (WHERE NOT c.is_closed) active_cases,
             count(*) FILTER (WHERE NOT c.is_closed AND c.next_action_due_date < CURRENT_DATE) overdue_actions,
             (SELECT count(*) FROM hearings h WHERE h.lg_case_id IN (SELECT id FROM cases c2 WHERE COALESCE(c2.assigned_legal_officer_id::text,'Unassigned') = COALESCE(c.assigned_legal_officer_id::text,'Unassigned'))
                AND h.hdate BETWEEN CURRENT_DATE AND CURRENT_DATE + 30) hearings30,
             count(*) FILTER (WHERE c.is_closed AND COALESCE(c.closed_date, c.updated_at::date) BETWEEN p_from AND p_to) closed_period,
             COALESCE(SUM(l.outstanding) FILTER (WHERE NOT c.is_closed), 0) outstanding
      FROM cases c LEFT JOIN liab l ON l.lg_case_id = c.id
      GROUP BY COALESCE(c.assigned_legal_officer_id::text, 'Unassigned')
    ) q
  )
  SELECT jsonb_build_object(
    'generated_at', now(),
