-- Server-side runtime endpoint registry (status callback derivation).
CREATE TABLE IF NOT EXISTS public.omni_comms_runtime_endpoint (
  endpoint_key text PRIMARY KEY,
  endpoint_url text NOT NULL,
  description text,
  updated_at timestamptz NOT NULL DEFAULT now()
);
GRANT SELECT ON public.omni_comms_runtime_endpoint TO authenticated;
GRANT ALL ON public.omni_comms_runtime_endpoint TO service_role;
ALTER TABLE public.omni_comms_runtime_endpoint ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS omni_comms_runtime_endpoint_read ON public.omni_comms_runtime_endpoint;
CREATE POLICY omni_comms_runtime_endpoint_read ON public.omni_comms_runtime_endpoint
...
VALUES ('twilio_status_callback',
        'https://xynceskeiiisiefqlgxo.supabase.co/functions/v1/omni-comms-webhook-twilio',
        'Server-derived Twilio StatusCallback endpoint. Never supplied by a caller.')
ON CONFLICT (endpoint_key) DO UPDATE SET endpoint_url = EXCLUDED.endpoint_url, updated_at = now();

-- WhatsApp claim: approved provider registration required (fail closed),
-- immutable variable mapping resolved to ContentVariables, server-derived callback.
CREATE OR REPLACE FUNCTION public.omni_comms_priv_dispatch_claim_whatsapp(
  p_worker text DEFAULT 'omni-comms-dispatch'::text,
  p_batch_limit integer DEFAULT 1,
  p_correlation_id text DEFAULT NULL::text,
  p_deployed_revision text DEFAULT NULL::text,
  p_execution_context text DEFAULT 'scheduler'::text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_from text;
  v_reg public.omni_comms_template_provider_registration%ROWTYPE;
  v_tv_status text;
  v_content_vars jsonb;
  v_callback text;
BEGIN
  ...
  LOOP
    ...
    v_deny := NULL; v_norm := NULL; v_hash := NULL; v_content_vars := '{}'::jsonb;
    ...
    IF v_deny IS NULL THEN
      ...
      ELSE
        ...
        -- Provider-neutral identity: canonical display_number, legacy fallback.
        v_from := coalesce(
          nullif(btrim(coalesce(v_identity.identity_config ->> 'display_number','')),''),
          nullif(btrim(coalesce(v_identity.identity_config ->> 'business_number','')),''));
    ...
    -- Automated business WhatsApp requires a PUBLISHED Omni template version and
    -- an APPROVED provider registration. Service windows are never guessed.
    IF v_deny IS NULL THEN
      v_reg := NULL; v_tv_status := NULL;
      IF v_job.template_version_id IS NULL THEN
        v_deny := 'template_version_missing';
      ELSE
        SELECT tv.status INTO v_tv_status
          FROM public.omni_comms_template_version tv WHERE tv.id = v_job.template_version_id;
        SELECT * INTO v_reg FROM public.omni_comms_template_provider_registration r
         WHERE r.template_version_id = v_job.template_version_id
           AND r.provider_account_id = v_account.id;
        IF coalesce(v_tv_status,'') <> 'published' THEN
          v_deny := 'template_version_not_published';
        ELSIF v_reg.id IS NULL THEN
          v_deny := 'provider_template_not_registered';
        ELSIF v_reg.provider_status <> 'approved'
           OR coalesce(btrim(v_reg.provider_template_ref),'') = '' THEN
          v_deny := 'provider_template_not_approved';
        ELSE
          SELECT coalesce(jsonb_object_agg(e.key,
                   coalesce(v_job.payload_snapshot #>> string_to_array(e.value, '.'), '')),
                 '{}'::jsonb)
            INTO v_content_vars
            FROM jsonb_each_text(v_reg.variable_mapping) e;
          IF EXISTS (
            SELECT 1 FROM jsonb_each_text(v_content_vars) x WHERE btrim(x.value) = ''
          ) THEN
            v_deny := 'template_variable_unresolved';
          END IF;
        END IF;
      END IF;
    END IF;
    ...
    INSERT INTO public.omni_comms_delivery_attempt (
      ...
      jsonb_build_object('channel','whatsapp','mode','queued',
                         ...
                         'provider_template_registration_id', v_reg.id,
                         'variable_count', (SELECT count(*) FROM jsonb_object_keys(v_content_vars)),
    ...
    v_claims := v_claims || jsonb_build_object(
      ...
      'content_sid', v_reg.provider_template_ref,
      'content_variables', v_content_vars,
