-- 1. Business-event ingestion is channel-neutral.
ALTER TABLE public.omni_comms_scheduler_run ALTER COLUMN channel DROP NOT NULL;

-- 2. Secure, tenant-scoped, canonical automation status.
CREATE OR REPLACE FUNCTION public.omni_comms_automation_status(p_organization_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
...
DECLARE
  v_actor uuid;
  v_service boolean := current_user IN ('service_role', 'postgres');
...
BEGIN
  -- Ordinary callers are authenticated Omni-Comms viewers and MUST scope the
  -- projection to one organisation. Only a proven service context may request
  ...
  -- Canonical dispatch-job vocabulary:
  --   pending | held | ready | leased | processing | completed | retry_wait
  --   | failed | cancelled
  SELECT jsonb_build_object(
    'waiting_jobs',      count(*) FILTER (WHERE status IN ('pending', 'held', 'ready', 'retry_wait')),
    'ready_jobs',        count(*) FILTER (WHERE status = 'ready'
                            AND (next_attempt_at IS NULL OR next_attempt_at <= now())),
    'held_jobs',         count(*) FILTER (WHERE status = 'held'),
    'retry_wait_jobs',   count(*) FILTER (WHERE status = 'retry_wait'),
    'currently_claimed', count(*) FILTER (WHERE status IN ('leased', 'processing')),
    'oldest_waiting_at', min(created_at) FILTER (WHERE status IN ('pending', 'held', 'ready', 'retry_wait')))
    INTO v_jobs
  ...
  -- Callback CONFIGURATION readiness is an endpoint fact, never an activity fact.
  SELECT EXISTS (
    SELECT 1 FROM public.omni_comms_channel_endpoint
     WHERE channel = 'email' AND endpoint_type = 'event_callback' AND status = 'active'
       AND (p_organization_id IS NULL OR organization_id = p_organization_id))
    INTO v_endpoint_ready;
  ...
  v_callbacks := coalesce(v_callbacks, '{}'::jsonb)
    || jsonb_build_object('callback_endpoint_ready', coalesce(v_endpoint_ready, false));

  v_jobs := v_jobs || jsonb_build_object('last_delivered_at', (v_callbacks->>'last_delivered_callback_at'));

  -- Global worker RUN health only. No tenant business counts here.
  ...
  RETURN jsonb_build_object(
    ...
    'callback_receiver', v_callbacks || jsonb_build_object(
      'healthy', coalesce(v_endpoint_ready, false)),
    'recent_runs', v_runs,
...
REVOKE ALL ON FUNCTION public.omni_comms_automation_status(uuid) FROM anon;
GRANT EXECUTE ON FUNCTION public.omni_comms_automation_status(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.omni_comms_automation_status(uuid) TO service_role;

REVOKE ALL ON FUNCTION public.omni_comms_automation_cron_evidence(text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.omni_comms_automation_cron_evidence(text) FROM anon;
GRANT EXECUTE ON FUNCTION public.omni_comms_automation_cron_evidence(text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.omni_comms_automation_cron_evidence(text) TO service_role;