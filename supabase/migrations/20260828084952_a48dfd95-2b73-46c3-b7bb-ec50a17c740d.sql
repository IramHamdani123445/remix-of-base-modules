...
BEGIN
  ...
  LOOP
    ...
    -- Every recipient destination on this channel must hash-match a rule.
    v_allow := true;
    v_any := false;
    FOR r_rec IN
...
REVOKE ALL ON FUNCTION public.omni_comms_priv_dispatch_certification_snapshot(uuid,uuid,uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.omni_comms_priv_dispatch_certification_snapshot(uuid,uuid,uuid) TO service_role;