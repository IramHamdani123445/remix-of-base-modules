-- Print provisioning: make max_messages_total explicit so the release-control ladder
-- constraint (hour <= day <= total) is satisfied by the seeded defaults.
...
BEGIN
  ...
  IF v_binding IS NULL THEN
    ...
    VALUES (p_organization_id, p_department_id, 'print', v_identity, v_account,
      v_endpoint, 1, 'draft', 'verified', 'service',
  ...
  UPDATE public.omni_comms_sender_provider_binding
     SET status='active', channel_endpoint_id=coalesce(channel_endpoint_id, v_endpoint),
         verification_status='verified', verification_source='service',
