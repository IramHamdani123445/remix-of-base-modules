...
BEGIN
  ...
  -- Producer visibility is a fact about the producer registry, never about
  -- whether Release Control happens to exist yet.
  SELECT count(*) INTO v_queued_bindings
   ...
   WHERE b.status = 'active'
     ...
     AND (
       ...
       OR (b.department_id IS NULL OR b.department_id = p_department_id)
     );

  SELECT count(*) INTO v_release_permitted
    FROM public.omni_comms_producer_event_binding b
    JOIN public.omni_comms_event_definition ed ON ed.id = b.event_definition_id
   WHERE b.status = 'active'
     AND 'queued' = ANY (b.allowed_modes)
  ...
  v_blocker := CASE
    WHEN v_queued_bindings = 0 THEN 'pilot_business_producer_not_selected'
    WHEN v_rel.id IS NULL THEN 'release_control_not_configured'
    WHEN v_release_permitted = 0 THEN 'pilot_business_producer_not_release_permitted'
    ELSE NULL END;
  ...
  RETURN jsonb_build_object(
    ...
    'queued_producer_binding_release_permitted', v_release_permitted,
