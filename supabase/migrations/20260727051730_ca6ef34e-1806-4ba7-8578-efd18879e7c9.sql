-- =========================================================
-- Forward Baseline Policies: version + hash + audit + RPC
-- =========================================================

-- 1) Add stable policy_version + policy_content_hash to both tables
...
ALTER TABLE public.communication_hub_event_review_policy
  ADD COLUMN IF NOT EXISTS policy_version bigint NOT NULL DEFAULT 1,
  ADD COLUMN IF NOT EXISTS policy_content_hash text;

-- 2) Immutable establishment audit table
CREATE TABLE IF NOT EXISTS public.communication_hub_forward_baseline_audit (
  ...
  send_policy_hash text,
  review_policy_id uuid,
  review_policy_version bigint,
  review_policy_hash text,
  actor uuid,
  reason text NOT NULL,
  effective_from timestamptz NOT NULL DEFAULT now(),
  established_at timestamptz NOT NULL DEFAULT now(),
  forward_baseline_only boolean NOT NULL DEFAULT true,
  typed_confirmation text NOT NULL,
  created_at timestamptz NOT NULL DEFAULT now()
);

GRANT SELECT ON public.communication_hub_forward_baseline_audit TO authenticated;
GRANT ALL ON public.communication_hub_forward_baseline_audit TO service_role;
ALTER TABLE public.communication_hub_forward_baseline_audit ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "comm_hub_admin_reads_fwd_baseline_audit"
  ON public.communication_hub_forward_baseline_audit;
CREATE POLICY "comm_hub_admin_reads_fwd_baseline_audit"
  ON public.communication_hub_forward_baseline_audit
  FOR SELECT
  TO authenticated
  USING (public.is_comm_hub_admin(auth.uid()));

-- Deny direct writes; only the security-definer RPC may insert.
DROP POLICY IF EXISTS "no_direct_writes_fwd_baseline_audit"
  ON public.communication_hub_forward_baseline_audit;

-- 3) Forward baseline RPC
...
BEGIN
  ...
  -- Event certification must exist, be live_manual_only, and match caller expectation
  ...
  IF v_evt_cert.id IS NULL THEN
    RAISE EXCEPTION 'event_certification_missing' USING ERRCODE='P0002';
  END IF;
  ...
  END IF;
  IF v_evt_cert.event_status <> 'live_manual_only' THEN
    RAISE EXCEPTION 'event_not_live_manual: status=%', v_evt_cert.event_status USING ERRCODE='P0001';
  END IF;

  -- Lifecycle guards on communication_hub_control
  SELECT * INTO v_ctrl FROM public.communication_hub_control LIMIT 1;
  ...
  -- Template version must match active mapping
  SELECT * INTO v_map
  ...
  IF v_map.id IS NULL THEN
    RAISE EXCEPTION 'template_map_missing' USING ERRCODE='P0002';
  END IF;
  ...
  -- Production lineage (for audit only)
  v_production_lineage_id := v_evt_cert.production_lineage_id;

  -- ============ SEND POLICY (idempotent create) ============
  SELECT * INTO v_sp
  ...
  IF v_sp.id IS NULL THEN
    ...
    v_send_hash := encode(digest(jsonb_build_object(
      ...
      'require_preview_before_manual_send', true,
      'require_typed_confirmation_for_send', false,
      'is_enabled', true
    )::text, 'sha256'),'hex');
  ...
  ELSE
    v_sp_id := v_sp.id;
    v_send_version := v_sp.policy_version;
    v_send_hash := v_sp.policy_content_hash;
  END IF;

  -- ============ REVIEW POLICY (idempotent create) ============
  SELECT * INTO v_rev
  FROM public.communication_hub_event_review_policy
  WHERE upper(module_code)=v_module AND upper(event_code)=v_event AND lower(channel)=v_channel
  LIMIT 1;
  ...
  IF v_rev.id IS NULL THEN
    ...
    v_review_hash := encode(digest(jsonb_build_object(
      'review_mode','preview_required',
      'preview_required', true,
      'allow_operator_edit_tokens', false,
      'allow_operator_edit_body', false,
      'allow_operator_change_recipient', false,
      'show_template_to_operator', true,
      'show_template_to_recipient_portal', false,
      'require_template_approval', true,
      'require_legal_approval', false,
      'require_business_approval', false,
      'approval_status','approved_internal',
      'approved_template_version_id', p_expected_template_version_id
    )::text, 'sha256'),'hex');
  ...
  ELSE
    v_rev_id := v_rev.id;
    v_review_version := v_rev.policy_version;
    v_review_hash := v_rev.policy_content_hash;
  END IF;

  -- ============ AUDIT ============
  INSERT INTO public.communication_hub_forward_baseline_audit (
    module_code, event_code, channel,
    production_lineage_id, event_certification_id,
    send_policy_id, send_policy_version, send_policy_hash,
    review_policy_id, review_policy_version, review_policy_hash,
    actor, reason, effective_from, established_at,
    forward_baseline_only, typed_confirmation
  ) VALUES (
    v_module, v_event, v_channel,
    v_production_lineage_id, v_evt_cert.id,
    v_sp_id, v_send_version, v_send_hash,
    v_rev_id, v_review_version, v_review_hash,
    v_uid, p_reason, now(), now(),
    true, p_typed_confirmation
  ...
  RETURN jsonb_build_object(
    ...
    'send_policy_id', v_sp_id,
    'send_policy_version', v_send_version,
    'send_policy_hash', v_send_hash,
    'review_policy_id', v_rev_id,
    'review_policy_version', v_review_version,
...
REVOKE ALL ON FUNCTION public.establish_comm_hub_forward_baseline_policies(
  text, text, text, uuid, uuid, text, text
) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.establish_comm_hub_forward_baseline_policies(
  text, text, text, uuid, uuid, text, text
) TO authenticated;