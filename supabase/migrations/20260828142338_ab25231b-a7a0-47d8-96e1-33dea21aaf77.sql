CREATE OR REPLACE FUNCTION public.omni_comms_priv_dispatch_certification_snapshot(p_actor_id uuid, p_request_id uuid, p_organization_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT coalesce(jsonb_agg(c.adapter_code ORDER BY c.adapter_code), '[]'::jsonb)
    INTO v_safe_adapters
    FROM public.omni_comms_channel_adapter_capability c
   WHERE c.enabled IS TRUE AND c.certification_safe IS TRUE;
  ...
  RETURN jsonb_build_object(
    ...
    'certification_safe_adapters', v_safe_adapters,
    'quarantined', EXISTS (
