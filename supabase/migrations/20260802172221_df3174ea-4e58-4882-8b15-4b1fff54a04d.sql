...
-- Enforce the remaining dispatch gates and evidence invariants.
-- No table is created or dropped. No delivery is enabled.
-- ============================================================================

-- ---------------------------------------------------------------------------
-- 1. Bounded claim-safety suspension helper (single entry point).
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.omni_comms_priv_dispatch_claim_safety_suspend(
...
DECLARE
  v_state text;
BEGIN
  -- Bounded trigger vocabulary. Ordinary configuration absence BEFORE a pilot
  -- is active is deliberately NOT a safety trigger.
  IF p_code IS NULL OR p_release_control_id IS NULL
     OR p_code NOT IN (
       ...
       'recipient_rule_integrity_failure',
       'volume_integrity_failure',
       'identity_not_operational',
       'identity_tenant_mismatch',
       'identity_department_mismatch',
       'endpoint_not_verified',
       'endpoint_tenant_mismatch',
       'endpoint_department_mismatch',
       'secret_reference_invalid',
       'credential_missing',
       'binding_ambiguous',
       'provider_identity_ambiguous',
       'payload_evidence_mismatch',
       'idempotency_evidence_mismatch') THEN
    RETURN jsonb_build_object('suspended', false, 'code', 'not_a_safety_trigger');
  END IF;
  ...
  END IF;
  IF v_state IS DISTINCT FROM 'controlled_pilot' THEN
    RETURN jsonb_build_object('suspended', false, 'code', 'pilot_not_active');
  END IF;

  RETURN public.omni_comms_priv_dispatch_suspend_pilot(
    p_release_control_id, 'claim_safety', p_code);
END;
$fn$;

REVOKE ALL ON FUNCTION public.omni_comms_priv_dispatch_claim_safety_suspend(uuid, text, uuid) FROM PUBLIC;
...
-- ---------------------------------------------------------------------------
-- 2. Claim: business_dispatch_enabled, strict batch limits, exact persisted
--    provider/endpoint resolution, automatic safety suspension.
-- ---------------------------------------------------------------------------
...
REVOKE ALL ON FUNCTION public.omni_comms_priv_dispatch_claim_email(text, integer, text, text, jsonb, text) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.omni_comms_priv_dispatch_claim_email(text, integer, text, text, jsonb, text) TO service_role;

-- ---------------------------------------------------------------------------
-- 3. Diagnostics: complete tenant, department, event and caller scoping.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- A queued producer binding only counts when it belongs to THIS
  -- organisation, a compatible department, and the effective release actually
  -- permits its event code and caller module. A binding in another tenant can
  -- never clear pilot_business_producer_not_selected.
...
REVOKE ALL ON FUNCTION public.omni_comms_dispatch_diagnostics(uuid, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.omni_comms_dispatch_diagnostics(uuid, uuid) TO authenticated, service_role;

-- ---------------------------------------------------------------------------
-- 4. Callback: truthful complaint / hard-bounce / soft-bounce lifecycle.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF v_scope = 'ambiguous' THEN
    -- An ambiguous callback suspends the related pilot ONLY when exactly one
    -- release can be resolved safely; otherwise the integrity event is
    -- recorded without guessing a release.
    SELECT DISTINCT a.release_control_id INTO v_suspend
      FROM public.omni_comms_delivery_attempt a
     WHERE a.provider_message_id = p_provider_message_id;
  ...
  -- Terminal harm: the message is failed and the pilot suspended BEFORE the
  -- request aggregate is recalculated, so the aggregate observes the truth.
  IF v_terminal THEN
    UPDATE public.omni_comms_message
     ...
     WHERE id = v_att.message_id
       AND status <> 'failed';
    UPDATE public.omni_comms_dispatch_job
       SET status = 'failed', hold_reason = CASE WHEN p_normalized_event_type = 'complained'
                                                 THEN 'complaint' ELSE 'hard_bounce' END,
           is_runnable = false, updated_at = now()
...
REVOKE ALL ON FUNCTION public.omni_comms_priv_dispatch_record_callback(text, text, text, text, text, timestamptz, jsonb, text, boolean) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.omni_comms_priv_dispatch_record_callback(text, text, text, text, text, timestamptz, jsonb, text, boolean) FROM anon, authenticated;
