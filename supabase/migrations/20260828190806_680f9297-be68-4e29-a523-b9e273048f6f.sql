-- Plain-text extraction helper for rendered communication HTML
CREATE OR REPLACE FUNCTION public.omni_comms_priv_html_to_text(_html text)
RETURNS text
LANGUAGE sql
IMMUTABLE
    ...
    btrim(
      regexp_replace(
        replace(replace(replace(replace(replace(
          regexp_replace(COALESCE(_html, ''), '<[^>]*>', ' ', 'g'),
          '&nbsp;', ' '), '&amp;', '&'), '&lt;', '<'), '&gt;', '>'), '&quot;', '"'),
        '\s+', ' ', 'g')
    ), '')
$$;

-- Derive a human title from rendered content
CREATE OR REPLACE FUNCTION public.omni_comms_priv_in_app_title(_subject text, _text text, _html text)
RETURNS text
LANGUAGE sql
IMMUTABLE
SET search_path TO 'pg_catalog', 'public'
AS $$
  SELECT COALESCE(
    ...
    (SELECT CASE
              ...
              WHEN length(split_part(body, '. ', 1)) BETWEEN 1 AND 90
                THEN split_part(body, '. ', 1)
              ELSE left(body, 90)
            END
       FROM (SELECT COALESCE(NULLIF(btrim(COALESCE(_text,'')),''),
                             public.omni_comms_priv_html_to_text(_html)) AS body) s),
    'Notification')
$$;

-- Projection now derives title/body from rendered content
CREATE OR REPLACE FUNCTION public.omni_comms_priv_dispatch_deliver_in_app(
  p_worker text DEFAULT 'omni-comms-in-app'::text,
...
BEGIN
  ...
  LOOP
    ...
    IF m.status = 'held' THEN
      UPDATE public.omni_comms_message SET status='queued' WHERE id = m.id;
    END IF;
    UPDATE public.omni_comms_message SET status='dispatching' WHERE id = m.id AND status='queued';
    ...
    IF v_user IS NULL THEN
      ...
      UPDATE public.omni_comms_dispatch_job
       ...
       WHERE id = j.id;
      UPDATE public.omni_comms_message SET status='failed' WHERE id = m.id AND status='dispatching';
    ...
    v_body := COALESCE(
                NULLIF(btrim(COALESCE(m.rendered_text,'')),''),
                public.omni_comms_priv_html_to_text(m.rendered_html),
                '');
    v_title := public.omni_comms_priv_in_app_title(m.rendered_subject, m.rendered_text, m.rendered_html);
    ...
    INSERT INTO public.in_app_notifications (
      ...
      v_user, v_title, v_body, v_action_url, 'omni_comms',
    ...
    UPDATE public.omni_comms_message SET status='accepted' WHERE id = m.id AND status='dispatching';
    UPDATE public.omni_comms_message SET status='delivered' WHERE id = m.id AND status='accepted';
...
-- Backfill existing in-app notifications projected from Omni-Comms
UPDATE public.in_app_notifications n
   SET title = public.omni_comms_priv_in_app_title(m.rendered_subject, m.rendered_text, m.rendered_html),
       body  = COALESCE(NULLIF(btrim(COALESCE(m.rendered_text,'')),''),
                        public.omni_comms_priv_html_to_text(m.rendered_html),
                        n.body)
  FROM public.omni_comms_message m
 WHERE n.omni_comms_message_id = m.id
   AND (COALESCE(n.title,'') IN ('', 'Notification') OR COALESCE(n.body,'') = '');