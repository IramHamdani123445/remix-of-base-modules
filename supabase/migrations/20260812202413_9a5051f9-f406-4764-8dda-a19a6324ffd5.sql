CREATE OR REPLACE FUNCTION public.omni_comms_priv_live_delivery_request(p_actor uuid, p_organization_id uuid, p_department_id uuid, p_channel text, p_deployed_revision text, p_intent text, p_correlation_id text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_ind jsonb; v_blockers text[]; v_from text;
BEGIN
  ...
  IF p_intent = 'disable' THEN
    ...
    UPDATE public.omni_comms_channel_release_control SET
      release_state = 'suspended', release_version = release_version + 1,
  ...
  IF v_rel.release_state IN ('test_only','controlled_pilot','suspended')
    ...
    -- Production LIVE has NO pilot-style lifetime: it runs until explicitly
    -- turned off, suspended, or invalidated by a genuine safety gate.
    UPDATE public.omni_comms_channel_release_control SET
  ...
  IF v_rel.proposed_state = 'live'
    ...
    IF v_rel.proposed_by = p_actor THEN
      RETURN jsonb_build_object('state','awaiting_approval','self_proposed', true,
        'blockers','[]'::jsonb);
    END IF;
    RETURN jsonb_build_object('state','approve_ready',
  ...
  UPDATE public.omni_comms_channel_release_control SET
    ...
    proposed_by = p_actor, proposed_at = now(),
    -- Governance of the PENDING approval only; not the lifetime of the release.
...
-- Normalise any existing production (live or proposed-live) release that was
-- given a pilot-style seven-day expiry by the previous implementation.
DO $$
BEGIN
  PERFORM set_config('omni_comms.live_transition', 'on', true);
  UPDATE public.omni_comms_channel_release_control
     SET release_expires_at = NULL
   WHERE (release_state = 'live' OR proposed_state = 'live')
     AND release_expires_at IS NOT NULL;
END $$;