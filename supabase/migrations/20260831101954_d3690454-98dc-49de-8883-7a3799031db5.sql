...
BEGIN
  ...
  -- Data scope resolved BEFORE any user filter: filters can only narrow.
  ...
  CREATE TEMP TABLE IF NOT EXISTS _wq_noop(x int);
  ...
  WITH base AS (
    SELECT 'Violation'::text AS work_type,
           ...
           (upper(COALESCE(v.status,'')) IN ('UNDER_REVIEW','ESCALATED')
             OR v.verification_decision = 'SENT_BACK') AS review_pending,
  ...
  v_total := COALESCE(v_total, (SELECT 0));

  -- Option lists come from configured reference data (cheap and stable).
    ...
    'owners', (
      ...
      FROM (
        SELECT DISTINCT COALESCE(i.profile_id::text, i.id::text) AS id,
               COALESCE(pr.full_name, i.inspector_code, i.legacy_inspector_code, left(i.id::text,8)) AS nm
