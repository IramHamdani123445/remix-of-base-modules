...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'grand_total', (SELECT count(*) FROM scoped),
    'sort', v_sort,
    'dir', v_dir,
    'rows', COALESCE((
      SELECT jsonb_agg(to_jsonb(r) ORDER BY r.rn) FROM ranked r
      WHERE r.rn > CASE WHEN p_export THEN 0 ELSE (v_page-1)*v_size END
        AND r.rn <= CASE WHEN p_export THEN v_size ELSE v_page*v_size END
    ), '[]'::jsonb),
    'options', CASE WHEN p_export THEN '{}'::jsonb ELSE jsonb_build_object(
      'actions', COALESCE((SELECT jsonb_agg(DISTINCT action ORDER BY action) FROM scoped WHERE action IS NOT NULL),'[]'::jsonb),
      'performers', COALESCE((SELECT jsonb_agg(DISTINCT performed_by ORDER BY performed_by) FROM scoped WHERE performed_by IS NOT NULL),'[]'::jsonb),
      'from_values', COALESCE((SELECT jsonb_agg(DISTINCT from_value ORDER BY from_value) FROM scoped WHERE from_value IS NOT NULL),'[]'::jsonb),
      'to_values', COALESCE((SELECT jsonb_agg(DISTINCT to_value ORDER BY to_value) FROM scoped WHERE to_value IS NOT NULL),'[]'::jsonb),
      'employers', COALESCE((SELECT jsonb_agg(jsonb_build_object('id', employer_id, 'name', COALESCE(employer_name, employer_id)) ORDER BY COALESCE(employer_name, employer_id))
          FROM (SELECT DISTINCT employer_id, employer_name FROM scoped WHERE employer_id IS NOT NULL) e),'[]'::jsonb),
      'violation_types', COALESCE((SELECT jsonb_agg(jsonb_build_object('id', violation_type_id, 'name', violation_type) ORDER BY violation_type)
          FROM (SELECT DISTINCT violation_type_id, violation_type FROM scoped WHERE violation_type_id IS NOT NULL) t),'[]'::jsonb),
      'violations', COALESCE((SELECT jsonb_agg(jsonb_build_object('id', violation_id, 'number', violation_number, 'employer_id', employer_id, 'employer_name', employer_name) ORDER BY violation_number DESC)
          FROM (SELECT DISTINCT violation_id, violation_number, employer_id, employer_name FROM scoped
                WHERE f_employer IS NULL OR employer_id = f_employer) vv),'[]'::jsonb)
    ) END,
    'summary', CASE WHEN p_export OR f_violation IS NULL THEN NULL ELSE (
      SELECT jsonb_build_object(
        ...
        'event_count', (SELECT count(*) FROM scoped s WHERE s.violation_id = v.id))
