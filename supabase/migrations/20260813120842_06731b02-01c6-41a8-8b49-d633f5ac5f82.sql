ALTER TABLE public.omni_comms_scheduler_run
  ADD COLUMN IF NOT EXISTS pipeline_stage text NOT NULL DEFAULT 'dispatch';

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'omni_comms_scheduler_run_pipeline_stage_chk') THEN
    ALTER TABLE public.omni_comms_scheduler_run
      ADD CONSTRAINT omni_comms_scheduler_run_pipeline_stage_chk
      CHECK (pipeline_stage IN ('business_event_ingest', 'dispatch'));
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS omni_comms_scheduler_run_stage_created_idx
  ON public.omni_comms_scheduler_run (pipeline_stage, created_at DESC);

CREATE OR REPLACE FUNCTION public.omni_comms_priv_record_ingest_run(
  p_worker text,
  p_metrics jsonb,
...
DECLARE
  v_id uuid;
  v_m jsonb := coalesce(p_metrics, '{}'::jsonb);
  v_safe jsonb;
BEGIN
  v_safe := jsonb_strip_nulls(jsonb_build_object(
    'events_scanned',          (v_m->>'events_scanned')::int,
    'events_claimed',          (v_m->>'events_claimed')::int,
    'events_processed',        (v_m->>'events_processed')::int,
    'events_no_communication', (v_m->>'events_no_communication')::int,
    'events_retried',          (v_m->>'events_retried')::int,
    'events_blocked',          (v_m->>'events_blocked')::int,
    'events_needs_review',     (v_m->>'events_needs_review')::int,
    'duration_ms',             (v_m->>'duration_ms')::int,
    'result_code',             left(coalesce(v_m->>'result_code', 'ok'), 60)));

  INSERT INTO public.omni_comms_scheduler_run (
    worker, execution_context, channel, pipeline_stage,
    scanned_jobs, claimed_jobs, blocker, detail)
  VALUES (
    left(coalesce(p_worker, 'omni-comms-business-event-ingest'), 120),
    'scheduler', NULL, 'business_event_ingest',
    coalesce((v_m->>'events_scanned')::int, 0),
    coalesce((v_m->>'events_claimed')::int, 0),
    nullif(left(coalesce(p_blocker, ''), 120), ''),
    v_safe)
...
REVOKE ALL ON FUNCTION public.omni_comms_priv_record_ingest_run(text, jsonb, text) FROM public;
GRANT EXECUTE ON FUNCTION public.omni_comms_priv_record_ingest_run(text, jsonb, text) TO service_role;
...
DECLARE
  ...
  v_ing_healthy boolean; v_dis_healthy boolean;
...
BEGIN
  ...
  BEGIN
    SELECT max(d.start_time) INTO v_ing_cron
      FROM cron.job_run_details d JOIN cron.job j ON j.jobid = d.jobid
     WHERE j.jobname = 'omni-comms-business-event-ingest-every-minute' AND d.status = 'succeeded';
    SELECT max(d.start_time) INTO v_dis_cron
      FROM cron.job_run_details d JOIN cron.job j ON j.jobid = d.jobid
     WHERE j.jobname = 'omni-comms-dispatch-every-minute' AND d.status = 'succeeded';
  EXCEPTION WHEN OTHERS THEN
    v_ing_cron := NULL; v_dis_cron := NULL;
  END;
  ...
  v_ing_healthy := v_ing_blocker IS NULL;
  v_dis_healthy := v_dis_blocker IS NULL;
  ...
  RETURN jsonb_build_object(
    'business_event_processor', jsonb_build_object(
      ...
      'healthy', coalesce(v_ing_healthy, false)) || coalesce(v_events, '{}'::jsonb),
    'delivery_processor', jsonb_build_object(
      ...
      'healthy', coalesce(v_dis_healthy, false)) || coalesce(v_jobs, '{}'::jsonb),
...
REVOKE ALL ON FUNCTION public.omni_comms_automation_status(uuid) FROM public;
GRANT EXECUTE ON FUNCTION public.omni_comms_automation_status(uuid) TO authenticated, service_role;