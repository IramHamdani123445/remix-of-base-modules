-- ─────────────────────────────────────────────────────────────
-- Compliance Payment Arrangements — Enterprise Monitoring Layer
-- ─────────────────────────────────────────────────────────────

CREATE TABLE IF NOT EXISTS public.ce_arrangement_ref (
...
GRANT SELECT ON public.ce_arrangement_ref TO authenticated;
GRANT ALL ON public.ce_arrangement_ref TO service_role;
ALTER TABLE public.ce_arrangement_ref ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS ce_arrangement_ref_read ON public.ce_arrangement_ref;
CREATE POLICY ce_arrangement_ref_read ON public.ce_arrangement_ref
  FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS ce_arrangement_ref_manage ON public.ce_arrangement_ref;
CREATE POLICY ce_arrangement_ref_manage ON public.ce_arrangement_ref
...
DROP TRIGGER IF EXISTS trg_ce_arrangement_ref_touch ON public.ce_arrangement_ref;
CREATE TRIGGER trg_ce_arrangement_ref_touch
  BEFORE UPDATE ON public.ce_arrangement_ref
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

INSERT INTO public.ce_arrangement_ref (domain, code, label, ui_tone, numeric_value, sort_order) VALUES
  ('STATUS','DRAFT','Draft','muted',NULL,10),
  ('STATUS','PENDING_APPROVAL','Pending approval','warning',NULL,20),
  ('STATUS','ACTIVE','Active','success',NULL,30),
  ('STATUS','BREACHED','Breached','destructive',NULL,40),
  ('STATUS','DEFAULTED','Defaulted','destructive',NULL,50),
  ('STATUS','COMPLETED','Completed','primary',NULL,60),
  ('STATUS','CANCELLED','Cancelled','muted',NULL,70),
  ('STATUS','SUPERSEDED','Superseded','muted',NULL,80),
  ('HEALTH','HEALTHY','Healthy','success',NULL,10),
  ('HEALTH','WARNING','Watch','warning',NULL,20),
  ('HEALTH','AT_RISK','At risk','warning',NULL,30),
  ('HEALTH','BREACHED','Breached','destructive',NULL,40),
  ('HEALTH','INACTIVE','Inactive','muted',NULL,50),
  ('FREQUENCY','WEEKLY','Weekly',NULL,NULL,10),
  ('FREQUENCY','BIWEEKLY','Fortnightly',NULL,NULL,20),
  ('FREQUENCY','MONTHLY','Monthly',NULL,NULL,30),
  ('FREQUENCY','QUARTERLY','Quarterly',NULL,NULL,40),
  ('THRESHOLD','APPROVAL_AGEING_DAYS','Approval ageing (days)',NULL,3,10),
  ('THRESHOLD','DRAFT_STALE_DAYS','Draft staleness (days)',NULL,10,20),
  ('THRESHOLD','DUE_SOON_DAYS','Instalment due soon (days)',NULL,7,30)
ON CONFLICT (domain, code) DO NOTHING;

-- ── Extended register view ───────────────────────────────────
DROP VIEW IF EXISTS public.ce_v_arrangement_register_ext;
CREATE VIEW public.ce_v_arrangement_register_ext AS
SELECT
  r.*,
  h.regno,
  c.case_number,
  ...
  a.breach_reason,
  a.updated_at,
  cp.superseded_by_arrangement_id,
  cp.superseded_from_arrangement_id,
  CASE WHEN COALESCE(r.total_arranged,0) > 0
       THEN round((COALESCE(r.total_paid,0) / r.total_arranged) * 100, 1)
       ELSE 0 END       AS paid_percent,
  CASE WHEN r.next_due_date IS NOT NULL
       THEN (r.next_due_date - CURRENT_DATE) END AS days_to_next_due,
  COALESCE(ref.label, r.status)   AS status_label,
  COALESCE(refh.label, r.health_status) AS health_label
FROM public.ce_v_arrangement_register r
LEFT JOIN public.ce_v_arrangement_health h ON h.arrangement_id = r.arrangement_id
LEFT JOIN public.ce_payment_arrangements a ON a.id = r.arrangement_id
LEFT JOIN public.ce_cases c ON c.id = r.case_id
LEFT JOIN public.core_payment_arrangement cp ON cp.legacy_ce_arrangement_id = r.arrangement_id
LEFT JOIN public.ce_arrangement_ref ref  ON ref.domain='STATUS' AND ref.code = r.status
LEFT JOIN public.ce_arrangement_ref refh ON refh.domain='HEALTH' AND refh.code = r.health_status;

GRANT SELECT ON public.ce_v_arrangement_register_ext TO authenticated, service_role;

-- ── Governed register RPC ────────────────────────────────────
...
BEGIN
  ...
  WITH base AS (
    ...
    SELECT jsonb_build_object(
        ...
        'reason', CASE
            WHEN s.is_breached THEN 'Breached — '||COALESCE(s.unresolved_breach_count,0)::text||' unresolved breach(es)'
...
REVOKE ALL ON FUNCTION public.ce_arrangement_register_v1(jsonb) FROM public;
GRANT EXECUTE ON FUNCTION public.ce_arrangement_register_v1(jsonb) TO authenticated, service_role;

-- ── Facets RPC ───────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.ce_arrangement_facets_v1()
RETURNS jsonb
LANGUAGE plpgsql
STABLE SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  ...
  RETURN jsonb_build_object(
    'statuses', COALESCE((SELECT jsonb_agg(jsonb_build_object('code',code,'label',label,'tone',ui_tone) ORDER BY sort_order)
                          FROM public.ce_arrangement_ref WHERE domain='STATUS' AND is_active),'[]'::jsonb),
    'health', COALESCE((SELECT jsonb_agg(jsonb_build_object('code',code,'label',label,'tone',ui_tone) ORDER BY sort_order)
                          FROM public.ce_arrangement_ref WHERE domain='HEALTH' AND is_active),'[]'::jsonb),
    'frequencies', COALESCE((SELECT jsonb_agg(jsonb_build_object('code',code,'label',label) ORDER BY sort_order)
                          FROM public.ce_arrangement_ref WHERE domain='FREQUENCY' AND is_active),'[]'::jsonb),
    'employers', COALESCE((SELECT jsonb_agg(e ORDER BY e->>'label')
                          FROM (SELECT DISTINCT jsonb_build_object('code', employer_id,
                                       'label', COALESCE(employer_name, employer_id)) AS e
                                FROM public.ce_v_arrangement_register_ext
                                WHERE employer_id IS NOT NULL LIMIT 500) q),'[]'::jsonb)
  );
END;
$function$;

REVOKE ALL ON FUNCTION public.ce_arrangement_facets_v1() FROM public;
GRANT EXECUTE ON FUNCTION public.ce_arrangement_facets_v1() TO authenticated, service_role;