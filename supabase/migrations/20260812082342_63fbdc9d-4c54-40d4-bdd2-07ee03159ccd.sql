-- Omni-Comms — recoverable aborted request.
--
-- A request that is accepted but then fails BEFORE any recipient is persisted
-- must not permanently poison its idempotency key. This migration:
--   1) adds a trusted compensating routine that marks such a request failed,
--   2) lets the send routine reuse a failed, recipient-less request when the
--      corrected retry carries a different fingerprint.
-- Neither routine contacts a provider or creates a runnable dispatch job.
...
BEGIN
  ...
  SELECT * INTO r FROM public.omni_comms_request
  ...
  -- Only an untouched request may be abandoned. Anything that already
  -- produced a recipient, message or dispatch job stays exactly as it is.
...
BEGIN
  ...
  IF FOUND THEN
    IF v_existing.request_fingerprint IS DISTINCT FROM p_request_fingerprint THEN
      -- A corrected retry after an aborted attempt. Reuse the SAME row only
      -- when the earlier attempt failed before producing anything at all.
