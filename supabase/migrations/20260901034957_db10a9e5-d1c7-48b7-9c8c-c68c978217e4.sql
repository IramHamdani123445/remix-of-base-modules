...
BEGIN
  ...
  RETURN jsonb_build_object(
    'notice', v_notice || jsonb_build_object('body', (SELECT body FROM public.ce_notices WHERE id=p_notice_id)),
    ...
    'audit', COALESCE((SELECT jsonb_agg(jsonb_build_object(
                            'at', a.created_at, 'actor', COALESCE(a.user_name, a.user_id::text), 'action', a.action,
                            'from_status', a.before_value, 'to_status', a.after_value) ORDER BY a.created_at DESC)
                       FROM public.system_audit_trail a
                       WHERE a.entity_type IN ('notice','ce_notices') AND a.entity_id = p_notice_id::text),'[]'::jsonb)
  );
END;
$$;
...
BEGIN
  ...
  WITH base AS (
    SELECT v.*,
      ...
      (v.notice_type_group = 'FINAL' AND v.status IN ('DRAFT','PENDING_APPROVAL','APPROVED')) AS final_awaiting_action,
    ...
    WHERE (v_search IS NULL OR (
      ...
      AND (v_due IS NULL OR CASE v_due
             WHEN 'OVERDUE' THEN s.response_state='OVERDUE'
    ...
    SELECT jsonb_build_object(
         ...
         'reason', CASE
             ...
             WHEN s.approval_stale THEN 'Pending approval beyond ' || v_appr_days::text || ' days'
             ...
             ELSE 'Response due shortly' END) AS a,
