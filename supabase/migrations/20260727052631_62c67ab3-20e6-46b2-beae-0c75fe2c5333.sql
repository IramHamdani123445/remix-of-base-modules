...
BEGIN
  ...
  IF v_evt_cert.id <> p_expected_event_certification_id THEN
    RAISE EXCEPTION 'event_certification_mismatch' USING ERRCODE='P0001';
  END IF;
  ...
  END IF;
  IF v_evt_cert.template_version_id IS DISTINCT FROM p_expected_template_version_id THEN
    RAISE EXCEPTION 'template_version_mismatch' USING ERRCODE='P0001';
  END IF;
