-- ===========================================================================
-- Omni-Comms Runtime Hardening — Part 2
--
-- Supersedes migration 20260731120019 (which mutated this function using
-- position()/replace() against its source text). That technique is not
-- auditable and is not reviewable in isolation. This migration restates the
-- COMPLETE function definition.
--
-- Functional delta vs. the previously deployed text-patched body:
--   * runtime function presence probe additionally reports
--     omni_comms_priv_authorize_runtime_actor and
--     omni_comms_priv_load_persisted_messages.
-- Everything else is byte-for-byte the deployed behaviour, including the
-- sender→binding→account→provider precedence fix.
-- ===========================================================================
CREATE OR REPLACE FUNCTION public.omni_comms_setup_readiness(
  p_organization_id uuid,
  p_department_id uuid DEFAULT NULL::uuid,
  p_event_definition_id uuid DEFAULT NULL::uuid,
...
BEGIN
  ...
  -- Sender-first provider precedence: the account/provider reported by
  -- readiness MUST be the one bound to the resolved sender identity, not the
  -- most recently updated global account.
...
'Omni-Comms Setup Wizard readiness evaluation. Restated in full (supersedes the text-patched migration 20260731120019). Sender-first provider precedence. Reports configuration facts only; never certifies runtime and never enables live delivery.';