CREATE OR REPLACE FUNCTION public.ia_office_holder_valid_at(p_status text, p_from date, p_to date, p_as_of date)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
SET search_path TO 'public'
AS $$
  SELECT coalesce(p_status,'') IN ('active','superseded')
     AND p_from <= p_as_of
     AND (p_to IS NULL OR p_to >= p_as_of);
$$;

REVOKE ALL ON FUNCTION public.ia_office_holder_valid_at(text, date, date, date) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ia_office_holder_valid_at(text, date, date, date) TO authenticated, service_role;
...
BEGIN
  ...
  ELSIF v_canon = 'LEAD_AUDITOR' THEN
    ...
    END IF;
    SELECT e.lead_auditor_id INTO v_profile
...
REVOKE ALL ON FUNCTION public.ia_resolve_escalation_recipient(text, uuid, uuid, uuid, date) FROM PUBLIC;
