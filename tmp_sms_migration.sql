CREATE OR REPLACE FUNCTION public.omni_comms_priv_dispatch_claim_generic(p_channel text, p_batch_limit integer, p_worker text, p_correlation_id text, p_execution_context text, p_deployed_revision text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  SELECT endpoint_url INTO v_callback FROM public.omni_comms_runtime_endpoint
     WHERE endpoint_key = CASE WHEN v_channel = 'voice' THEN 'twilio_voice_status_callback' ELSE 'twilio_status_callback' END;
  SELECT endpoint_url INTO v_ivr_action FROM public.omni_comms_runtime_endpoint WHERE endpoint_key = 'twilio_voice_ivr_action';
  ...
  LOOP
    ...
    IF v_deny IS NULL THEN
      ...
      ELSE -- webhook
        v_sub := public.omni_comms_priv_resolve_webhook_subscription(
