...
| 2 | Harness had 16 scenarios; three required semantics were unproven | Added `department_access_rejection`, `registered_but_unauthorised_module_rejection` and `concurrent_idempotency_semantics`. Edge-revision binding was **not** added as a scenario — it is a preflight condition (see §2) |
...
| 5 | Rendering assertions tolerated unrendered messages | Asserts every message carries a 64-hex checksum, has no unresolved required slots or blockers, that response checksums equal persisted checksums, and that each message reached its **mode-derived terminal status** (`dry_run_completed`, `shadow_completed` or `held`) — the transient `rendered` state is never accepted as terminal evidence |
...
— 19 scenarios execute on every run (`EXPECTED_SCENARIO_COUNT = 19`), of which
`shadow_creates_held_jobs_only` and `queued_creates_held_jobs_only` are
generated by one mode loop.
...
**Edge-revision binding is a preflight condition, not a scenario.** It is
evaluated before a single fixture is created; when the deployed revision does
not equal the certified commit (or is unpublished under
`OMNI_COMMS_REQUIRE_EDGE_REVISION=1`) the run is refused (exit 2). It is never
counted as a passing scenario.

The tenant-isolation fixtures are also preflight conditions:
`OMNI_COMMS_TEST_FOREIGN_DEPARTMENT_ID` must exist in `core_department`, belong
to `OMNI_COMMS_TEST_ORGANIZATION_ID`, differ from
`OMNI_COMMS_TEST_DEPARTMENT_ID`, and carry **no active
`core_staff_assignments` entitlement** for the authorised actor. Accordingly
`department_access_rejection` requires exactly HTTP 403,
`omni_comms.result.v1`, blockers `["department_access_denied"]` and zero
persisted request rows; `department_organization_mismatch` is not accepted
there, because cross-organisation ownership is certified separately by
