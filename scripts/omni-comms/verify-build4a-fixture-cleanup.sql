...
--
-- Usage:
--   psql "$DB" -v cert_org='<uuid>' -v cert_namespace='<NS>' \
...
BEGIN
  ...
  -- The certification organisation must be a namespaced fixture tenant.
  IF NOT EXISTS (
    SELECT 1 FROM public.core_organization
     WHERE id = v_org AND org_code LIKE v_ns || '%'
  ) THEN
    RAISE EXCEPTION 'SCOPE: cert_org is not inside the certification namespace';
  END IF;

  -- 1. no Build 4A or runtime rows may remain for the certification tenant
  FOREACH v_tbl IN ARRAY ARRAY[
    ...
    EXECUTE format(
      'SELECT count(*) FROM public.%I WHERE organization_id = $1', v_tbl
  ...
  SELECT count(*) INTO v_n FROM public.core_department WHERE organization_id = v_org;
  IF v_n > 0 THEN
    RAISE EXCEPTION 'CLEANUP: % certification department fixtures remain', v_n;
  END IF;

  -- 3. no injected fault mechanism may remain anywhere
