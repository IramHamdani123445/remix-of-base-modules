...
export const OMNI_COMMS_READINESS_MANIFEST: OmniCommsReadinessManifest = {
  ...
  foundationStatus: [
    ...
    { item: 'Health Readiness (Phase 3)',                    state: 'Verified', note: 'Phase 3 — the Readiness tab remains source-controlled and static: registries, architecture boundaries, approved routes, object catalogue, planned integrations and foundation status. It is never merged with live environment status.' },
    { item: 'Health Live Diagnostics (Phase 3)',             state: 'Verified', note: 'Phase 3 — the Live Diagnostics tab reads ACTUAL deployed configuration and runtime state through four bounded SECURITY DEFINER RPCs (omni_comms_health_summary, _catalogue, _runtime, _permissions; owner postgres, pinned search_path, PUBLIC/anon revoked, authenticated granted, omni_comms.view enforced, mandatory organisation scope, department ownership validated, no mutation, no provider contact, no secret return). Eight diagnostic categories, derived posture, prioritized recommended actions, manual refresh plus an off-by-default 60s auto-refresh. Lives on the existing Health route as a tab — permanent route count remains seven.' },
    ...
    { item: 'Controlled Dry-Run surface (Phase 5)',          state: 'Verified', note: 'Phase 5 — a controlled dry-run test surface is available as a tab on the existing Overview route (?view=dry-run); the permanent route count remains seven. Execution is gated by the server-side omni_comms_controlled_dry_run_gate aggregate (feature flag plus omni_comms.operate capability) AND by dry_run_ready from omni_comms_setup_readiness. Payloads are validated authoritatively by omni_comms_validate_dry_run_payload against the published contract; validation is invalidated whenever the organisation, department, event or payload changes. Submission goes only through the canonical sendCommunication() façade with mode=dry_run, channel=email, one synthetic example.com recipient, a prefixed idempotency key and correlation id. Post-run invariants confirm zero dispatch jobs, zero delivery attempts, no provider contact and no email send, and the result deep links to the Operations request detail.' },
    { item: 'Dry-run boundary enforcement (Phase 5)',        state: 'Verified', note: 'Phase 5 — architecture Rule 14 OMNI_CONTROLLED_DRY_RUN_BOUNDARY rejects private RPC use, non-approved RPCs, direct table access, direct Edge Function invocation, runtime internal imports, any non dry_run mode literal, hardcoded non-example.com addresses, queue/dispatch/shadow/live escalation vocabulary, secret material and Legacy Communication Hub references on the dry-run surface. Negative fixtures prove detection; the repository-wide check reports 0 unbaselined violations.' },
